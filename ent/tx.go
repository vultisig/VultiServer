...
func (tx *Tx) init() {
	tx.ECDSAPub = NewECDSAPubClient(tx.config)
	tx.EcdsaLocalData = NewEcdsaLocalDataClient(tx.config)
	tx.EddsaLocalData = NewEddsaLocalDataClient(tx.config)
	tx.PaillierSK = NewPaillierSKClient(tx.config)
...
// applies a query, for example: ECDSAPub.QueryXXX(), the query will be executed
